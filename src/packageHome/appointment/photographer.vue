    ...
    <div class="photographer-main">
      <scroll-view class="photographer-main-scroll-view" scroll-y="true">
        <up-skeleton :loading="loading" :rows="3">
          ...
          <div
            class="photographer-main-card"
            v-for="(item, index) in photographerList"
            :key="item.userId"
            @click="handleSelectPhotographer(item.userId, index)"
            :class="{ selected: selectedPhotographer === item.userId }"
          >
            <div class="card">
              <div class="card-1">
                <div class="left">
              ...
              <div class="card-3">
                <div class="left">
                  <span>约拍价</span>
                  <span class="price"> <span style="font-size: 32rpx"> ￥ </span> {{ item.price || '待定' }}</span>
                </div>
                <div class="right" @click="handleDetail(item.userId)"> TA的作品 </div>
              </div>
            </div>
          </div>
        </up-skeleton>
  ...
  .photographer {
    ...
    &-tag {
      ...
      &-item {
        ...
        height: 80rpx;
        border-radius: 12rpx;
        border: 2rpx solid rgba(40, 40, 40, 0.5);
        font-size: 28rpx;
