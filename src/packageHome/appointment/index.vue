...
<script setup lang="ts">
  ...
  const generateTimeSlots = (startHour: number, endHour: number, endMinute: number = 0) => {
    ...
    while (currentHour < endHour || (currentHour === endHour && currentMinute < endMinute)) {
      const endHourValue = currentHour + 1;
      const timeSlot = `${String(currentHour).padStart(2, '0')}:${String(currentMinute).padStart(2, '0')}-${String(endHourValue).padStart(2, '0')}:${String(currentMinute).padStart(2, '0')}`;
      slots.push(timeSlot);
  ...
  // 定义时间段
  const morningTimes = computed(() => generateTimeSlots(8, 12, 0));
  const afternoonTimes = computed(() => generateTimeSlots(12, 18, 30));
  ...
  const updateWeekDays = (date: number) => {
    ...
    const selectedDayIndex = days.findIndex(day => {
      ...
      return dayDate.getDate() === selectedDate.getDate() && dayDate.getMonth() === selectedDate.getMonth() && dayDate.getFullYear() === selectedDate.getFullYear();
    });
  ...
  const handleTimeSlotClick = (slot: 'morning' | 'afternoon' | 'evening') => {
    ...
    if (selectedTime.value) {
      ...
      const isInCurrentSlot = (slot === 'morning' && hours >= 8 && hours < 12) || (slot === 'afternoon' && hours >= 13 && hours < 18) || (slot === 'evening' && hours >= 19 && hours < 22);
  ...
  const confirmDate = (e: any) => {
    ...
    if (selectedDayIndex !== -1 && !isDateDisabled(weekDays.value[selectedDayIndex].date)) {
      ...
      // 获取当前时间，根据时间段自动选择合适的时间段
      const now = new Date();
      const currentHour = now.getHours();

      if (currentHour < 12) {
        timeSlot.value = 'morning';
        const availableTime = morningTimes.value.find(time => !isTimeDisabled(time));
        if (availableTime) handleTimeClick(availableTime);
      } else if (currentHour < 18) {
        timeSlot.value = 'afternoon';
        const availableTime = afternoonTimes.value.find(time => !isTimeDisabled(time));
        if (availableTime) handleTimeClick(availableTime);
      } else {
        timeSlot.value = 'evening';
        const availableTime = eveningTimes.value.find(time => !isTimeDisabled(time));
        if (availableTime) handleTimeClick(availableTime);
      }
    }
  };
