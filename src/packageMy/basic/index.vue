<template>
    ...
    <up-skeleton rows="6" :loading="loading">
      <div class="basic-card" v-if="user">
        ...
        <div class="basic-card-item" @click="openPopup('生日', user.birthday, 'birthday')">
          ...
          <span class="content">{{ user.birthday ? formatDate(user.birthday, 'YYYY-MM-DD') : '未设置' }}</span>
        </div>
        <div class="basic-card-item" @click="openPopup('宝宝纪念日', user.babyBirthday, 'babyBirthday')">
          <span class="basic-card-item-title">宝宝生日</span>
          <span class="content">{{ user.babyBirthday ? formatDate(user.babyBirthday, 'YYYY-MM-DD') : '未设置' }}</span>
        </div>
        <div class="basic-card-item" @click="openPopup('结婚纪念日', user.weddingAnniversary, 'weddingAnniversary')">
          ...
          <span class="content">{{ user.weddingAnniversary ? formatDate(user.weddingAnniversary, 'YYYY-MM-DD') : '未设置' }}</span>
        </div>
  ...
  <up-popup :show="show" @close="show = false" :round="10" closeable class="relative">
    <view class="popup">
      ...
      <div v-if="currentField === 'sex'">
        <div class="desc-btn mb-[55rpx]" :class="{ active: popupContent == 1 }" @click="popupContent = 1"> 男 </div>
        <div class="desc-btn" :class="{ active: popupContent == 2 }" @click="popupContent = 2"> 女 </div>
      </div>
  ...
  <up-popup :show="picShow" @close="picShow = false" :round="10" closeable class="relative">
    <view class="popup">
      ...
      <div class="desc-btn !text-color-[#ba2636] mb-[55rpx]" @click="previewPicture"> 查看图片 </div>
      <div class="desc-btn" @click="updatePicture"> 更新图片 </div>
    </view>
  </up-popup>
  ...
  <t-cropper mode="ratio" :imageUrl="chooseAvatarUrl" :width="500" :height="500" :delay="150" @cancel="chooseAvatarUrl = ''" @confirm="updateAvatar"></t-cropper>
</template>
...
<script setup lang="ts">
  ...
  const openPopup = (title: string, content: any, field: string) => {
    active.value = user.value.sex;
    if (['birthday', 'babyBirthday', 'weddingAnniversary'].includes(field) && content === null) {
