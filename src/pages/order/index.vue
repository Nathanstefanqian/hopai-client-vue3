<template>
  <div class="order w-100vw h-100vh flex">
    ...
    <scroll-view class="order-main-scrollview" scroll-y="true" @scrolltolower="loadMore">
            ...
            <div class="order-item" v-for="(item, index) in orderList" :key="item.id" v-else>
              ...
              <div class="order-item-desc">
                ...
                <div class="order-item-desc-three">
                  <div class="more" :style="isMoreButtonDisabled(item.orderStatus) ? 'opacity: 0.5; cursor: not-allowed;' : ''" @click="handleMoreClick(item)">更多操作</div>
                  <div class="right">
    ...
    <u-action-sheet :show="showActionSheet" :actions="actions" @close="showActionSheet = false" @select="handleActionSelect" />
...
<script setup lang="ts">
  ...
  import { submitPayOrders } from '@/api/order';
  ...
  const handleClick = (e: any) => {
    ...
    noMore.value = false;
    orderList.value = [];
    const currentTab = tabList.value[e.index];
  ...
  const handleMoreClick = async (item: any) => {
    // 如果订单状态为已取消、退款中或退款成功，则不执行任何操作
    if (isMoreButtonDisabled(item.orderStatus)) return;
    currentOrder.value = item;
    showActionSheet.value = true;
  };

  const isMoreButtonDisabled = (orderStatus: number) => {
    return [10, 20, 30, 100].includes(orderStatus);
  };
  ...
  const handleOrderButton = (item: any) => {
    ...
    switch (item.orderStatus) {
      case 0:
        ...
        buttonConfig.onClick = () => handlePay(item);
        break;
      case 7:
          ...
          &-desc {
            ...
            &-three {
              ...
              .right {
                ...
                .comment {
                  margin-right: 20rpx;
                  border: 0.33px solid rgba(40, 40, 40, 0.5);
                  border-radius: 16rpx;
                  padding: 12rpx 24rpx;
                }
